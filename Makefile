...
show-deps:
	cat Makefile | grep -E 'wget' | sed -Ee 's/\twget\s(https.*)/\1/g'
...
tomcat:
	...
	if [ "$action" = "1" ]; then
		...
		sudo bash -c "curl  $(THIS_DIR)/conf/tomcat-service >/etc/systemd/system/tomcat.service"
